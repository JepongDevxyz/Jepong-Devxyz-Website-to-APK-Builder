...
const app=path.join(project,'platforms/android/app');
const javaDir=path.join(app,'src/main/java/com/jepongdevxyz/cordovacontrols');
...
const run=spawnSync(
  process.execPath,
  ['scripts/patch-android-platform.mjs','--build-id',id,'--engine','cordova'],
  {cwd:root,encoding:'utf8'}
);

if(run.status!==0){
  throw new Error(`Cordova patch failed:\n${run.stderr}`);
}
...
for(const token of required){
  ...
  if(!compact.includes(normalized)){
    throw new Error(`cordova control missing: ${token}`);
  }
}

for(const control of ['navigationToolbar','externalLinks','downloadManager']){
  const capability=getCapability('cordova','controls',control);

  if(capability.status!==CAPABILITY_STATUS.VERIFIED){
    throw new Error(`cordova ${control} must be Verified`);
  }
  if(!capability.compileVerified || !capability.runtimeVerified){
    throw new Error(`cordova ${control} Verified metadata is stale`);
  }
  if(!getSelectableFeatureIds('cordova','controls').includes(control)){
    throw new Error(`cordova ${control} is not selectable`);
  }
}

console.log('✓ Cordova browser controls regression');
