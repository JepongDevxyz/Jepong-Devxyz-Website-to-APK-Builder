...
if(cmd==='prepare') {
  ...
  if(cfg.engine==='native') { writeNative(cfg,out,false); patchCrossEngineBrowserUx(cfg,out); patchModernBackManifest(out); }
  if(cfg.engine==='gecko') { writeNative(cfg,out,true); patchGeckoStartup(cfg,out); patchCrossEngineBrowserUx(cfg,out); patchModernBackManifest(out); }
