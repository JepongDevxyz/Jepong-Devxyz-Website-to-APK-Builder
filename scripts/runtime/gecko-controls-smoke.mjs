...
async function run(...args){
  ...
  try{
    ...
    throw new Error(
      `ADB command failed or timed out: adb ${args.join(' ')} :: ${error?.message||error}`
    );
  }
}
...
<body><h1>GECKO_PAGE_2</h1>${stateScript('page2')}</body>
</html>`;
...
async function dumpUi(timeout=10000){
  ...
  throw new Error(
    `UI dump unavailable after retries: ${lastError}`
  );
}
...
async function waitForDesc(description,timeout=10000){
  ...
  while(Date.now()-started<timeout){
    try{
      ...
      last=ui;
      if(ui.includes(`content-desc="${description}"`)){
        return ui;
      }
    }catch(error){
  ...
  throw new Error(
    `UI content description did not appear: ${description}; last=${String(last).slice(0,280)}`
  );
}
...
async function waitForAnyText(values,timeout=7000){
  ...
  while(Date.now()-started<timeout){
    try{
      const ui=await dumpUi(
        Math.min(
          3000,
          Math.max(900,timeout-(Date.now()-started))
        )
      );
      last=ui;
      for(const value of values){
        if(
          ui.includes(`text="${value}"`) ||
          ui.includes(`content-desc="${value}"`)
        ){
          return {ui,value};
        }
      }
  ...
  throw new Error(
    `None of the expected UI texts appeared: ${values.join(', ')}; last=${String(last).slice(0,280)}`
  );
}
...
function boundsForAttribute(ui,attribute,value){
  ...
  for(const node of nodes){
    ...
    if(bounds){
      return bounds.slice(1,5).map(Number);
    }
  }

  throw new Error(
    `Could not locate ${attribute}=${value} in UI hierarchy`
  );
}

async function tapDesc(description){
  const ui=await waitForDesc(description);
  const [x1,y1,x2,y2]=boundsForAttribute(
    ui,
    'content-desc',
    description
  );
...
async function screenSize(){
  ...
  const match=
    /Physical size:\s*(\d+)x(\d+)/.exec(output) ||
    /(\d+)x(\d+)/.exec(output);

  if(!match){
    throw new Error(
      `Could not determine emulator screen size: ${output}`
    );
  }
...
async function waitForRequest(pathname,minCount,timeout=45000){
  ...
  while(Date.now()-started<timeout){
    if((requests.get(pathname)||0)>=minCount){
      return;
    }
    await sleep(120);
  }
  ...
  throw new Error(
    `HTTP request did not occur: ${pathname} x${minCount}; observed=${requests.get(pathname)||0}`
  );
}
...
function resumedActivityLine(dump){
  return String(dump||'')
    ...
    .find(
      line=>
        /ResumedActivity|topResumedActivity/i.test(line) &&
        /ActivityRecord/i.test(line)
    ) || '';
}
...
async function waitForResumed(predicate,timeout=10000){
  ...
  while(Date.now()-started<timeout){
    ...
    if(predicate(last,dump)){
      return {line:last,dump};
    }
    await sleep(250);
  }

  throw new Error(
    `Expected resumed activity did not appear; last=${last||'<none>'}`
  );
}
...
async function diagnostics(label){
  ...
  try{
    const logs=await run('logcat','-d','-t','1200');
    const relevant=logs
      .split('\n')
      .filter(line=>
        /Gecko|AndroidRuntime|MainActivity|Jepong|DownloadManager|DocumentsUI|ActivityTaskManager/i.test(line)
      )
      .slice(-260)
      .join('\n');
...
try{
  ...
  for(const label of [
    'Back','Forward','Home','Refresh','Share'
  ]){
    if(!ui.includes(`content-desc="${label}"`)){
      throw new Error(
        `Gecko navigation toolbar missing at runtime: ${label}`
      );
    }
  }

  const foreground=await foregroundDump();
  if(!foreground.includes(pkg)){
    throw new Error(
      'Gecko app did not reach foreground/activity stack'
    );
  }
  ...
  await waitForResumed(
    line=>line.includes(`${pkg}/.MainActivity`),
    10000
  );
  ...
  stage('file-picker');
  await tapWeb(0.47);
  const picker=await waitForResumed(
    (line,dump)=>
      /documentsui/i.test(line) ||
      /documentsui/i.test(dump),
    12000
  );
  ...
  if(!clear.includes('Success')){
    throw new Error(
      `Unable to clear Gecko app state before exit test: ${clear}`
    );
  }
  await run('shell','am','start','-W','-n',activity);
  ...
  await run('shell','input','keyevent','4');
  const cancel=await waitForAnyText(['CANCEL','Cancel'],10000);
  const exit=await waitForAnyText(['EXIT','Exit'],10000);
  const afterBack=await foregroundDump();
  if(!afterBack.includes(pkg)){
    throw new Error(
      'Gecko home Back left the app before explicit exit confirmation'
    );
  }
  ...
  if(
    logs.includes('FATAL EXCEPTION') &&
    logs.includes(pkg)
  ){
    throw new Error(
      'Gecko app fatal exception detected during runtime smoke'
    );
  }

  console.log(
    '✓ Gecko deterministic navigation/external/file/download/exit runtime smoke'
  );
}catch(error){
